...
export const PlanSelector: React.FC = () => {
  ...
  const handleSettingsSave = (newSettings: PomodoroSettings) => {
    ...
    if (customHours) {
      ...
      const selectedPlan = PLAN_OPTIONS.find(
        (option) => option.id !== "custom" && !showCustomHours && showCustomSettings,
      )
      if (selectedPlan) {
  ...
  if (showCustomHours) {
    return <CustomHours onSave={handleCustomHoursSave} onCancel={handleCustomHoursCancel} />
  }

  if (showCustomSettings) {
    return (
      <CustomSettings onSave={handleSettingsSave} onCancel={handleSettingsCancel} initialSettings={defaultSettings} />
    )
  }

  return (
    <div className="plan-selector">
      <h1 className="plan-selector__title text-3xl font-bold mb-8 text-center">What's your plan for today?</h1>
      <div className="plan-selector__options grid gap-4 md:grid-cols-2">
        {PLAN_OPTIONS.map((option) => (
          <button
            key={option.id}
            onClick={() => handlePlanSelect(option.id)}
            className="plan-selector__option flex flex-col items-center justify-center p-6 border-2 border-gray-200 rounded-lg hover:border-gray-400 transition-colors"
          >
            <h2 className="plan-selector__option-title text-xl font-semibold mb-2">{option.label}</h2>
            <p className="plan-selector__option-hours text-3xl font-bold mb-2">
              {option.id === "custom" ? "?" : `${option.hours}h`}
            </p>
            <p className="plan-selector__option-description text-sm text-gray-600 text-center">{option.description}</p>
          </button>
        ))}
      </div>
    </div>
