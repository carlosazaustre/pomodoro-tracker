...
export const TimerButton: React.FC<TimerButtonProps> = ({
  ...
  const getButtonClasses = () => {
    const baseClasses =
      "timer-button relative w-full h-16 rounded-lg font-medium transition-all overflow-hidden";
    ...
    return `${baseClasses} bg-gray-100 hover:bg-orange-100 text-gray-800`;
  };
  ...
  return (
    <button
      ...
      onClick={handleClick}
      disabled={!isActive || block.status === "completed"}
    >
      {block.status === "running" && (
        <div
          className="timer-button__progress absolute left-0 top-0 bottom-0 bg-orange-700 z-0"
          style={{ width: `${progress}%` }}
        />
      )}
      <span className="timer-button__label relative z-10">
        {getButtonLabel()}
      </span>
      <span className="timer-button__type text-xs absolute bottom-1 right-2 opacity-70 z-10">
        {block.type === "work"
          ? "Work"
          : block.type === "short-break"
          ? "Short Break"
          : "Long Break"}
      </span>
    </button>
